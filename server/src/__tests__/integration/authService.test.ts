...
describe('Registering', () => {
  ...
  it('Throws an error when passwords arent the same', async () => {
    ...
    await expect(authService.handleRegister(input)).rejects.toBeInstanceOf(
      UserInputError,
    );
  });
  ...
  it('Throws an error if the email is taken', async () => {
    ...
    //Create the second
    await expect(authService.handleRegister(input)).rejects.toEqual(
      new UserInputError(
...
describe('Logging in', () => {
  ...
  it('Throws an error with invalid credentials', async () => {
    ...
    await expect(
      authService.handleLogin('bob@bobiscool.com', 'password'),
    ).rejects.toEqual(new UserInputError('Invalid Credentials'));
    await expect(
      authService.handleLogin('bob@bob.com', 'password123'),
    ).rejects.toEqual(new UserInputError('Invalid Credentials'));
  });
});
